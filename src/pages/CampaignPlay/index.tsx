...
const CampaignPlay: React.FC = () => {
    ...
    // TDB -> This will be implemented in the future
    const handleOpenFullReport = () => {
        openContentSideMenu(<FullReportSummary />);
    };

    const handleDownloadLeads = ({ campaignContentId }: { campaignContentId: string }) => {
    ...
    const formattedRowData = reportData?.[0]?.campaign_content?.map((content) => ({
        ...
        fullReport: (
            <Button disabled variant="outline" onClick={() => handleOpenFullReport()}>
                Conferir
            </Button>
        ),
        contactsArchive: (
