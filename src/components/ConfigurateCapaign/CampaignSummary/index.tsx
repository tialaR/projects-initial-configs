...
const CampaignSummary: React.FC<CampaignSummaryProps> = ({ data, onCancel, onConfirm }) => {
    const { activateCampaign, loading, error, data: activateCampaignData } = useActivateCampaign();
    ...
    return (
        <S.Container>
            ...
            {error && <ErrorMessage>Erro ao tentar ativar campanha: {error}</ErrorMessage>}
        </S.Container>
    );
};
